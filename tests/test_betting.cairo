use blitzr::{
    BetStatus, IBlitzrDispatcher, IBlitzrDispatcherTrait, IMockERC20Dispatcher,
...
fn USER_CHARLIE() -> ContractAddress {
    starknet::contract_address_const::<'CHARLIE'>()
}
...
fn deploy_blitzr_contract(token_address: ContractAddress) -> IBlitzrDispatcher {
    ...
    let mut constructor_calldata = array![token_address.into()];
...
fn test_multiple_users_same_match() {
    ...
    assert(
        blitzr.get_user_bet_by_index(USER_CHARLIE(), 0) == charlie_bet_id, 'Wrong Charlie bet ID',
    );
}
...
fn test_create_bet_completed_match() {
    ...
    // Owner updates the bet result
    start_cheat_caller_address(blitzr.contract_address, OWNER());
