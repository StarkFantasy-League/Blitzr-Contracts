...
pub trait IBlitzr<TContractState> {
    ...
    fn get_match_pool_info(
        self: @TContractState, match_id: u256,
    ) -> (u256, u256, u256, u256); // (total, team_a_pool, team_b_pool, draw_pool)

    // Core betting functions
    fn create_bet(
        ref self: TContractState, match_id: u256, amount: u256, predicted_result: felt252,
    ) -> u256;

    // Validation function
    fn update_bet_result(ref self: TContractState, bet_id: u256, match_result: felt252);
    fn add_authorized_resolver(ref self: TContractState, resolver: ContractAddress);
    fn remove_authorized_resolver(ref self: TContractState, resolver: ContractAddress);
    ...
    fn is_authorized_resolver(self: @TContractState, account: ContractAddress) -> bool;
}
...
pub mod Blitzr {
    ...
    struct Storage {
        ...
        authorized_resolvers: Map<ContractAddress, bool>,
    }
    ...
    fn constructor(
        ...
        // Set owner (this will be your admin address)
    ...
    impl Blitzr of super::IBlitzr<ContractState> {
        ...
        fn create_bet(
            ref self: ContractState, match_id: u256, amount: u256, predicted_result: felt252,
        ) -> u256 {
            ...
            self.emit(BetCreated { bet_id, user: caller, match_id, amount, predicted_result });
        ...
        fn update_bet_result(ref self: ContractState, bet_id: u256, match_result: felt252) {
            ...
            // Check if caller is owner OR authorized resolver
            let is_owner = self.owner.read() == caller;
            let is_resolver = self.authorized_resolvers.read(caller);

            assert(is_owner || is_resolver, 'Unauthorized');
        ...
        fn calculate_winnings(self: @ContractState, bet_id: u256) -> u256 {
            ...
            // Calculate winnings: (user_bet / winning_pool) * (total_pool - protocol_fee) -
            // protocol fee
        ...
        fn add_authorized_resolver(ref self: ContractState, resolver: ContractAddress) {
            let caller = get_caller_address();
            assert(self.owner.read() == caller, 'Only owner can add resolvers');

            self.authorized_resolvers.write(resolver, true);
        }

        fn remove_authorized_resolver(ref self: ContractState, resolver: ContractAddress) {
            let caller = get_caller_address();
            assert(self.owner.read() == caller, 'Only owner can remove resolvers');

            self.authorized_resolvers.write(resolver, false);
        }

        //////////// USER FUNCTIONS ////////////
        // Note: Users can call this directly - NO ADMIN NEEDED!
        ...
        fn withdraw_winnings(ref self: ContractState, bet_id: u256) {
            ...
            self.emit(WinningsWithdrawn { bet_id, user: caller, amount: winnings });
        }
        ...
        fn is_authorized_resolver(self: @ContractState, account: ContractAddress) -> bool {
            self.authorized_resolvers.read(account)
        }
    }
    ...
    impl Private of PrivateTrait {
        fn _validate_user_balance(ref self: ContractState, user: ContractAddress, amount: u256) {
